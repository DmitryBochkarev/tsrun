...
fn test_symbol_uniqueness() {
    ...
    assert_eq!(eval("Symbol('foo') === Symbol('foo')"), JsValue::Boolean(false));
}
...
fn test_symbol_for() {
    ...
    assert_eq!(eval("Symbol['for']('foo') === Symbol['for']('foo')"), JsValue::Boolean(true));
    // But different from Symbol()
    assert_eq!(eval("Symbol['for']('foo') === Symbol('foo')"), JsValue::Boolean(false));
}
...
fn test_symbol_key_for() {
    ...
    // Note: "for" is a reserved keyword so use bracket notation
    assert_eq!(eval("Symbol.keyFor(Symbol['for']('test'))"), JsValue::from("test"));
...
fn test_symbol_to_string() {
    assert_eq!(eval("Symbol('foo').toString()"), JsValue::from("Symbol(foo)"));
...
fn test_well_known_symbols_are_unique() {
    ...
    assert_eq!(eval("Symbol.iterator === Symbol.iterator"), JsValue::Boolean(true));
    assert_eq!(eval("Symbol.toStringTag === Symbol.toStringTag"), JsValue::Boolean(true));
}
