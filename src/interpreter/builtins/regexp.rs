...
pub fn regexp_test(_interp: &mut Interpreter, this: JsValue, args: Vec<JsValue>) -> Result<JsValue, JsError> {
...
pub fn regexp_exec(interp: &mut Interpreter, this: JsValue, args: Vec<JsValue>) -> Result<JsValue, JsError> {
    let (pattern, flags) = get_regexp_data(&this)?;
    let input = args.first().cloned().unwrap_or(JsValue::Undefined).to_js_string().to_string();
